...
{{-- PHẦN 2: BẢO MẬT & TRUY CẬP --}}
<div class="mt-5">
    <h6 class="form-section-title"><i class="bi bi-shield-lock me-2"></i> Bảo mật & Danh sách thí sinh</h6>
    ...
    <div class="row g-4">
        ...
        <div class="col-md-6">
            ...
            <div class="input-group">
                ...
                <input type="text" name="password" class="form-control border-start-0 ps-0" placeholder="Mật khẩu vào phòng">
            </div>
        </div>

        {{-- Upload File CSV --}}
        <div class="col-md-6">
            <label class="form-label">Tải lên danh sách thí sinh (.csv)</label>
            <div class="upload-area shadow-sm border-primary border-opacity-10" id="uploadContainer" style="position: relative; border: 2px dashed #cbd5e1; padding: 20px; text-align: center; border-radius: 12px; cursor: pointer;">
                <div class="upload-icon text-primary" style="font-size: 2rem;"><i class="bi bi-file-earmark-spreadsheet"></i></div>
                <div class="fw-bold text-dark small mb-1" id="fileNameDisplay">Nhấn để chọn file CSV (Excel)</div>
                <div class="text-muted small" style="font-size: 0.7rem;">Chỉ học sinh trong file mới được vào thi</div>
                
                <input type="file" name="student_file" id="student_file_input" 
                       class="position-absolute top-0 start-0 w-100 h-100 opacity-0 cursor-pointer" 
                       accept=".csv" style="cursor: pointer;">
            </div>
            <div class="form-text mt-2 small text-muted">
                * Lưu ý: File Excel khi lưu hãy chọn định dạng <strong>CSV (Comma delimited)</strong>. Cột đầu tiên là <strong>Email</strong>.
            </div>
        </div>
    </div>
</div>
...
<script>
    document.getElementById('student_file_input').addEventListener('change', function(e) {
        if (e.target.files.length > 0) {
            var fileName = e.target.files[0].name;
            document.getElementById('fileNameDisplay').innerHTML = '<i class="bi bi-check-circle-fill text-success me-1"></i> ' + fileName;
            document.getElementById('uploadContainer').style.backgroundColor = '#f0fff4';
        }
    });
</script>
</x-layouts.shared>