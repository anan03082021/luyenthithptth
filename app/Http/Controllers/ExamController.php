...
if ($isWhitelisted || $hasAccessByPassword || $sessionId == 0) {
    ...
    public function submitExam(Request $request, $sessionId)
    {
        ...
        // 4. Lưu câu trả lời (Hỗ trợ câu hỏi nhiều ý như Đúng/Sai)
        // 4. Lưu câu trả lời (Đã thêm logic lọc Orientation)
if ($request->has('answers')) {
    foreach ($request->answers as $rawQuestionId => $answerData) {
        $answerIds = is_array($answerData) ? $answerData : [$answerData];

        foreach ($answerIds as $answerId) {
            // Load quan hệ question để check orientation
            $answer = Answer::with('question')->find($answerId);
            
            if ($answer && $answer->question) {
                $question = $answer->question;
                
                // 1. Chuẩn hóa orientation của câu hỏi
                $qOri = strtolower(trim($question->orientation ?? ''));
                if ($qOri === '') $qOri = 'chung';

                // 2. Kiểm tra điều kiện lưu:
                // - Là câu CHUNG
                // - HOẶC là câu thuộc môn học sinh đã chọn (selectedElective)
                // - HOẶC nếu là Luyện tập ($isPractice) thì lưu hết để tránh sót
                $shouldSave = ($qOri === 'chung') 
                              || ($qOri === $selectedElective) 
                              || $isPractice;
        ...
        $attempt->refresh(); // Lấy lại điểm số vừa tính từ service
        $totalQuestions = Question::whereHas('exams', function($q) use ($examId) {
            $q->where('exams.id', $examId);
        })->whereNull('parent_id')->count(); // Đếm tổng số câu hỏi chính của đề

        if ($totalQuestions > 0 && $attempt->total_score > 10) {
            // Nếu điểm đang là "số câu đúng" (lớn hơn 10), quy đổi về thang 10
            $finalScore = ($attempt->total_score / $totalQuestions) * 10;
            $attempt->update(['total_score' => round($finalScore, 2)]);
        }
...
// =========================================================================
    ...
    public function showOfficialResult($id)
    {
        ...
        $uniqueAnswers = $attempt->attemptAnswers->unique('id');
