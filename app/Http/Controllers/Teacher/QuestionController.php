...
// Đảm bảo đã có dòng này ở đầu file: use Illuminate\Http\Request;

public function index(Request $request)
{
    // 1. Khởi tạo truy vấn (Lưu ý: đổi 'creator' thành 'creator' nếu bạn đã thêm hàm creator() vào Model)
    $query = Question::with(['topic', 'competency', 'cognitiveLevel', 'answers', 'children.answers', 'creator'])
        ->whereIn('type', ['single_choice', 'true_false_group']);

    // 2. Lọc theo Tab (filter)
    $filter = $request->input('filter', 'all');
    
    $query->when($filter == 'mine', function ($q) {
        // SỬA: creator_id -> user_id
        return $q->where('user_id', auth()->id());
    })
    ->when($filter == 'others', function ($q) {
        // SỬA: creator_id -> user_id
        return $q->where('user_id', '!=', auth()->id());
    })
    ->when($filter == 'thpt', function ($q) {
        return $q->whereHas('exams', function($e) {
            $e->where('title', 'LIKE', '%THPT%');
        });
    });
    ...
    return view('teacher.questions.index', compact('questions', 'topics', 'competencies', 'levels', 'filter'));
}
    ...
    public function destroy($id)
    {
        try {
            ...
            if ($question->user_id !== Auth::id()) {
                return redirect()->back()->with('error', 'Bạn không thể xóa câu hỏi của người khác.');
            }
            // Xóa đáp án
    ...
    public function edit($id)
    {
        ...
        if ($question->user_id !== Auth::id()) {
            return redirect()->route('teacher.questions.index')
                ->with('error', 'Bạn không có quyền chỉnh sửa câu hỏi của giáo viên khác.');
        }
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($question->user_id !== Auth::id()) {
            return back()->with('error', 'Hành động không được phép!');
        }
